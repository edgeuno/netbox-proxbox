...
def tag(netbox_vm):
    print("[DEBUG] Get current tags")
    # Get current tags
    tags = netbox_vm.tags

    # Get tag names from tag objects
    print("[DEBUG] Get tag names from tag objects")
    tags_name = []
    for tag in tags:
    ...
    print("[DEBUG] If Proxbox not found int Netbox tag's list, update object with the tag.")
    if create.extras.tag().name not in tags_name:
        ...
        print("[DEBUG] Save new tag to object")
