...
class VMPortScannerSyncMultithread:
    ...
    def get_service_from_port(vm, ip, port, timeout=3):
        ...
        is_open, t_host, t_port = VMPortScannerSyncMultithread.test_port_number(host, port, timeout)
    ...
    def get_services_from_ports(vm, ip, limit=100):
        ...
        for i in range(0, pages):
            try:
                ...
                # dispatch all tasks
                futures = [executor.submit(VMPortScannerSyncMultithread.get_service_from_port, vm, ip, port, 4) for port in
                           ports_subset]
                ...
                for future in as_completed(futures):
                    ...
                    if value is not None:
                        service = VMPortScannerSyncMultithread.set_service_to_vm(vm, ip, value)
    ...
    def process_ports(host, ports, limit=100):
        ...
        # report a status message
        print(f'Start Scanning {host}...')
        # result = VMPortScannerSyncMultithread.test_port_number(host, 22021)
        pages = math.ceil(len(ports) / (limit if limit is not None or limit != 0 else len(ports)))
        open_ports = []
        ...
        for i in range(0, pages):
            try:
                ...
                # dispatch all tasks
                futures = [executor.submit(VMPortScannerSyncMultithread.test_port_number, host, port, 4) for port in ports_subset]
                # report results in order
                for future in as_completed(futures):
                    value = future.result()
    ...
    def process_ip(vm, ip):
        ...
        ports_open = VMPortScannerSyncMultithread.process_ports(host, ports_range, 1000)
        # print(ports_open)
        services = []
        for port in ports_open:
            s = VMPortScannerSyncMultithread.set_service_to_vm(vm, ip, port)
            if s is not None:
                services.append(s)
    ...
    def process_vm(vm):
        ...
        ips = VMPortScannerSyncMultithread.get_ips_for_vm(vm)
        if ips is None or len(ips) < 1:
            return vm
        services = []
        executor = ThreadPoolExecutor(max_workers=len(ips))
        # futures = [executor.submit(VMPortScannerSyncMultithread.get_services_from_ports, vm, ip, 10) for ip in ips]
        futures = [executor.submit(VMPortScannerSyncMultithread.process_ip, vm, ip) for ip in ips]
    ...
    def run_bulk(vms):
        ...
        futures = [executor.submit(VMPortScannerSyncMultithread.process_vm, vm) for vm in vms]
        # report results in order
        for future in as_completed(futures):
            value = future.result()
            output.append(value)
    ...
    def run(tenants):
        ...
        netbox_vms = VMPortScannerSyncMultithread.get_vm_by_tenant(tenants)
        ...
        result = VMPortScannerSyncMultithread.run_bulk([netbox_vms[0]])
        # for vms_subset in list_vms:
        #     value = VMPortScannerSyncMultithread.run_bulk(vms_subset)
        #     if value is not None:
        #         result = result + value
        # futures = [pool.submit(VMPortScannerSyncMultithread.run_bulk(vms_subset)) for vms_subset in list_vms]
        # for future in as_completed(futures):
        #     value = future.result()
        #     if value is not None:
        #         result = result + value
        ...
        #         break
        # if vm:
        #     result = VMPortScannerSyncMultithread.run_bulk([vm])
        #     print(result)
        # for i in netbox_vms:
        #     if i.name == 'E1-cpanel.edgeuno.com':
        #         vm = i
        #         break
        # if vm:
        #     result = VMPortScannerSyncMultithread.process_vm(vm)
        #     print(result)
